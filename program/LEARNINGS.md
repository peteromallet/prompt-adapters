...
- **T2 (vs prompted_baseline) is the load-bearing gate.** If the adapter doesn't beat prompting on quality, the extra architecture is wasted. T2/T3 verdicts at n<20 are unreliable; 001's n=4 TIE flipped to n=20 FAIL on the same data. Need n≥20 probes and evaluation at intermediate checkpoints to distinguish noise from regression.
- **T3 surface-feature metrics are too noisy and should not gate 003.** Need an LLM-judge-based style metric. For now, LLM-judge directly comparing "which text sounds more like this reference" is probably the better T3 instrument.
- **Capability tests (α-blending, strength-dial) are more diagnostic than quality tests at small scale.** Because the architectural claim is about affordances prompting can't replicate. At any scale, if α-blending produces garbage rather than interpolation, the latent space isn't continuous — the program has a fundamental issue, regardless of how good T2 looks.
